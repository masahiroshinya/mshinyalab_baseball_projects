...
- [ ] MATLABのカレントフォルダが `2026-0604/MATLAB/` になっている
- [ ] `s8_forstudy_scratch8.m` をエディターで開いた
...
- [ ] `AnalogFs` の値を確認する（何 Hz か？）
- [ ] `Force1` の行数が「試行時間 [s] × AnalogFs [Hz]」と一致することを確認する
...
- [ ] Figure 1 にグラフが表示されることを確認する
- [ ] 試行の前後に「ほぼ一定の値が続く区間（静止期）」があることを確認する
- [ ] スイング中に値が大きく変化することを確認する
- [ ] プレート1とプレート2で、波形の形が異なることを確認する（片方が増えると、もう片方は減る？）
