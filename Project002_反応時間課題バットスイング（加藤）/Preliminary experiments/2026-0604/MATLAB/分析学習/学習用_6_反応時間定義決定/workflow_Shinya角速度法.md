...
- [ ] MATLABのカレントフォルダが `MATLAB/` になっていることを確認する
- [ ] `s6_forstudy_scratch6_rt_angular_velocity.m` が新規作成できていることを確認する
...
- [ ] `Data` が読み込まれることを確認する
- [ ] `fs` の値をコマンドウィンドウで確認する（例：200 Hz など）
- [ ] フィールド名の一覧に `bottom` と `top` が含まれることを確認する
- [ ] `bottom` が含まれない場合は、グリップ端に相当するマーカー名を特定して控えておく：`________`
