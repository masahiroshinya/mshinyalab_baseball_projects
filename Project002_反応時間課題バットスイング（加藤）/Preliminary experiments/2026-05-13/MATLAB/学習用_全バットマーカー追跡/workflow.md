...
- [ ] STEP 1 のコードを作成し、実行してエラーが出ないことを確認する。
...
- [ ] STEP 2 のコードを追記し、ワークスペースに `Data` が作成されることを確認する。
...
- [ ] コマンドウィンドウにマーカー名一覧が表示されることを確認する。
- [ ] バット関連マーカーとして使うフィールド名をメモする。
...
- [ ] `batMarkerNameArray` に実在するマーカー名だけを入れる。
- [ ] `top` と `bottom` 以外に追加すべきバットマーカー名を確認する。
...
- [ ] フィルター後のマーカー構造体 `M` が作成されることを確認する。
...
- [ ] `BatMarkerVelocity` に各マーカーの速度波形が保存されることを確認する。
- [ ] `BatMarkerPeakVelocity` に各マーカーのピーク速度が保存されることを確認する。
...
- [ ] 各バットマーカーの欠損フレーム数を確認する。
- [ ] 欠損が多いマーカーを解析の主指標に使わない方針を検討する。
...
- [ ] 全バットマーカーの速度波形が1つの図に重ねて表示されることを確認する。
- [ ] `top` 以外のマーカーでもスイングらしい速度上昇が見えるか確認する。
...
- [ ] LED点灯を0秒とした速度波形が表示されることを確認する。
- [ ] LED後の速度立ち上がりが複数マーカーで同じ傾向を示すか確認する。
...
- [ ] `Result` に保存すべきフィールドを決める。
- [ ] 既存の `PeakVelTop` を残すかどうかを確認する。
- [ ] 複数試行集計で扱いやすい保存形式を検討する。

---

## 完了後の動作確認チェックリスト

* [ ] `fieldnames(Data.Markers)` で、実データに含まれる全マーカー名を確認した。
* [ ] `top` と `bottom` 以外のバット関連マーカー名を特定した。
* [ ] 全バットマーカーに同じフィルター処理を適用した。
* [ ] 各バットマーカーの合成速度を計算した。
* [ ] 各バットマーカーのピーク速度を確認した。
* [ ] 各バットマーカーの欠損フレーム数を確認した。
* [ ] 速度波形を重ねて表示し、不自然なマーカーがないか確認した。
* [ ] LED点灯を0秒とした時間軸で、各マーカーの速度立ち上がりを確認した。
* [ ] `m3_analyze_single_trial.m` に反映する保存項目とフィールド名の方針を決めた。
