...
> **`'extrap'` オプションについて（重要）**
>
> `interp1` のデフォルト動作は**内挿（interpolation）**のみです。
> データの先頭や末尾に NaN がある場合、既知点の「外側」にあたるため、
> `'extrap'` なしだと NaN のまま埋まりません。
>
> ```
> 既知点: t = [5, 6, 7, 8, ...]
> NaN箇所: t = [1, 2, 3, 4]  ← 外挿（extrap）が必要な範囲
> ```
>
> `'extrap'` を第5引数に指定することで、外側の NaN も線形外挿で補完されます。
> これを忘れると NaN が残り、`filtfilt` でエラーになります。
...
- [X] NaN補間ブロックを実行してもエラーが出ないことを確認する
- [X] `filt_all_fields` 関数を使ってフィルタリングできることを確認する（エラーが出ない）
- [X] `M` 構造体に `bottom` と `top` フィールドが含まれることを確認する
...
- [X] 以下の検証コードを実行し、`e_long` のノルムが 1.0 に近いことを確認する
...
- [X] 最大値・最小値が 1.000000 に近い値（例: 0.999999〜1.000001）であることを確認する
...
**ステップ1：単位ベクトルは「向きしか変わらない」**

単位ベクトルはノルム（大きさ）が常に 1 です。
大きさは変わらず、**向きだけが変わります**。
...
つまり、単位ベクトルの変化 = 純粋な回転の変化です。

---

**ステップ2：時計の針で考える**

時計の秒針を「単位ベクトル」だと考えます（長さ = 1）。
...
秒針が 1秒間に θ [rad] 回転すると、**秒針の先端**はどれだけ動くか？

```
弧の長さ = 半径 × 角度
         = 1 × θ      ← 半径が 1 なので、角度と同じ値になる
```
...
バット長軸の単位ベクトル `e_long` は「バットが今どの方向を向いているか」です。

```
スイング前           スイング中
   ↑ e_long(t)          ↗ e_long(t+dt)
   |                   /
   ●──────バット       ●──────バット
（グリップ）         （グリップ）
```

バットが速く回転するほど、`e_long` の向きが速く変わります。
`‖de_long/dt‖` はその「変化の速さ」= 角速度です。

| 変数               | 意味                           | 単位   |
| ------------------ | ------------------------------ | ------ |
| `e_long`         | バット長軸の向き（大きさ = 1） | 無次元 |
| `de_long/dt`     | 向きの変化速度                 | rad/s  |
| `‖de_long/dt‖` | 変化の速さ =**角速度**   | rad/s  |

半径 1 の円弧では「距離 = 角度」なので、先端の速度と角速度が同じ数値になります。
これが「単位ベクトルの微分のノルム = 角速度」の理由です。
...
- [X] スイング試行で最大値が 300 deg/s を大きく超えることを確認する（例：1000 deg/s 以上）
- [X] `omega_deg` が `[nFrames × 1]` の列ベクトルであることを確認する
...
Go 信号と NoGo 信号は **別々の列**に記録されています：

| 列                | 信号        | 電圧  |
| ----------------- | ----------- | ----- |
| `LEDData(:, 2)` | Go キュー   | +5 V  |
| `LEDData(:, 1)` | NoGo キュー | −5 V |

列2だけを見ると NoGo 試行では信号が検出できないため、2列を個別に検索します。
...
>
> - Go（列2）は正電圧なので `> 2` で検出
> - NoGo（列1）は負電圧なので `< -2` で検出
>
> `abs(led) > 2` を1列だけに使う方法では、もう一方の列の信号が拾えません。

### 確認ポイント

- [X] `tCueMarker` の値が表示されることを確認する
- [X] `cueText` が `'Go'` または `'NoGo'` になることを確認する
- [X] Go 試行・NoGo 試行それぞれで正しく検出されることを確認する
...
- [X] `tSwingOnset` の値が表示されることを確認する（Go 試行の場合）
- [X] `tSwingOnset > tCueMarker` であることを確認する（スイング開始は LED 点灯後のはず）
- [X] `omega_deg(tSwingOnset)` の値が 300 deg/s に近いことを確認する
...
- [X] RT がおおよそ 150〜600 ms 程度であることを確認する
- [X] 値が極端に小さい（例: 50 ms 以下）または大きい（例: 1000 ms 以上）場合は、データやフレームレートを見直す
...
- [X] 図が表示されることを確認する
- [X] 角速度波形が LED 点灯後（t > 0 秒）に上昇していることを確認する
- [X] 赤い破線（300 deg/s）と赤い縦線（スイング開始）が表示されることを確認する
- [X] 縦線の位置が角速度の立ち上がりと一致しているかを目視確認する
- [X] 縦線が LED 点灯前（t < 0）に来ていないことを確認する
...
- [X] 複数の Go 試行で実行し、`tSwingOnset` が NaN でないことを確認する
- [X] RT が 150〜600 ms 程度であることを確認する
- [X] グラフで角速度が 300 deg/s を明確に超えていることを確認する
...
- [X] STEP 0: ファイルを新規作成し、カレントフォルダを設定した
- [X] STEP 1: データの読み込みができた。`bottom` と `top` マーカーを確認した
- [X] STEP 2: `filt_all_fields` でマーカーをフィルタリングした
- [X] STEP 3: `e_long`（長軸単位ベクトル）を計算し、ノルムが 1.0 であることを確認した
- [X] STEP 4: `omega_deg` を計算し、最大値がスイング試行で 300 deg/s を大きく超えることを確認した
- [X] STEP 5: `tCueMarker` と `cueText` が正しく取得された
- [X] STEP 6: Go 試行でスイング開始が検出され、`tSwingOnset > tCueMarker` であることを確認した
- [X] STEP 7: RT が 150〜600 ms 程度であることを確認した
- [X] STEP 8: グラフで縦線の位置を目視確認した
- [X] STEP 9: 複数の Go 試行と NoGo 試行で動作を確認した
