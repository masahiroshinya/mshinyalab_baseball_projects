...
allRT    = [RT_simple ; RT_gonogo];
alllabel = [repmat({'simple'}, length(RT_simple), 1); ...
            repmat({'gonogo'}, length(RT_gonogo), 1)];

boxplot(allRT, alllabel, 'GroupOrder', {'simple', 'gonogo'});

ylabel('反応時間（ms）');
title(sprintf('Subject %02d：条件別反応時間\nsimple：%.0f±%.0f ms / gonogo：%.0f±%.0f ms', ...
    iSubject, mean_simple, std_simple, mean_gonogo, std_gonogo));
grid on
