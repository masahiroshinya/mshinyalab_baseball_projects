...
%     figure 2 : 合成速度の時系列（条件ごと・全試行重ね描き／NoGo・Stop 除外）
%     figure 3 : Vx のピーク（散布図：ピーク時刻 × ピーク値）
%     figure 4 : Vx の時系列（条件ごと・全試行重ね描き／NoGo・Stop 除外）
%     figure 5 : 条件別ピーク速度の箱ひげ図（NoGo・Stop 除外）
%
% 入力:
...
% スイング動作を伴わない試行（NoGo・gostop の Stop）は、
% 散布図・箱ひげ図・時系列のいずれからも除外する。
% 参考として散布図にだけ薄く出したい場合は下記を true にする。
SHOW_EXCLUDED_IN_SCATTER = false ;   % figure 1・3 に NoGo / Stop を含めるか
ExcludedCueTextArray     = {'NoGo', 'Stop'} ;   % 集計から外すキュー種別
...
for iCondition = 1:nCondition
    ...
    tExcl      = [] ;   pExcl     = [] ;    % NoGo / Stop 用（合成速度）
    tExclX     = [] ;   pExclX    = [] ;    % NoGo / Stop 用（Vx）
    nExcluded  = 0 ;                        % 除外した試行数
    ...
    for iTrial = 1:nTrial
        ...
        if ismember(Result.CueText, ExcludedCueTextArray)
            % NoGo / Stop：スイングを抑制する試行なので集計から外す
            nExcluded = nExcluded + 1 ;
            tExcl(end+1)  = tPeak  ; pExcl(end+1)  = peak  ;        %#ok<SAGROW>
            tExclX(end+1) = tPeakX ; pExclX(end+1) = peakX ;        %#ok<SAGROW>
        else
    ...
    fprintf('%-7s Go: n=%2d, ピーク %.1f±%.1f m/s, 時刻 %.2f±%.2f s（NoGo/Stop %d 試行を除外）\n', ...
        ConditionNameArray{iCondition}, numel(peakList), ...
        mean(peakList), std(peakList), mean(tPeakList), std(tPeakList), nExcluded) ;

    figure(1)
    scatter(tPeakList, peakList, 45, ConditionColor{iCondition}, 'filled', ...
        'DisplayName', ConditionNameArray{iCondition}) ;
    if SHOW_EXCLUDED_IN_SCATTER && ~isempty(tExcl)
        scatter(tExcl, pExcl, 45, ConditionColor{iCondition}, 'x', 'LineWidth', 1.5, ...
            'DisplayName', [ConditionNameArray{iCondition} ' (NoGo/Stop)']) ;
    end

    figure(3)
    scatter(tPeakListX, peakListX, 45, ConditionColor{iCondition}, 'filled', ...
        'DisplayName', ConditionNameArray{iCondition}) ;
    if SHOW_EXCLUDED_IN_SCATTER && ~isempty(tExclX)
        scatter(tExclX, pExclX, 45, ConditionColor{iCondition}, 'x', 'LineWidth', 1.5, ...
            'DisplayName', [ConditionNameArray{iCondition} ' (NoGo/Stop)']) ;
    end
...
% -----------------------------------------------------------------------
% figure 5：条件別ピーク速度の箱ひげ図（Go 試行のみ。NoGo・Stop は除外済み）
% -----------------------------------------------------------------------
figure(5) ; clf

% データがない条件を GroupOrder に渡すと boxplot がエラーになるため除く
hasData = ismember(ConditionNameArray, boxLabel) ;
...
subplot(1,2,1)
boxplot(boxVel, boxLabel, 'GroupOrder', ConditionNameArray(hasData)) ;
...
subplot(1,2,2)
boxplot(boxVelX, boxLabel, 'GroupOrder', ConditionNameArray(hasData)) ;
set(gca, 'YLim', [yLo yHi]) ;
ylabel('Vx のピーク (m/s)') ;
...
% -----------------------------------------------------------------------
% figure 2・4：時系列（条件ごとに subplot、全試行を重ね描き。NoGo・Stop は除外）
% -----------------------------------------------------------------------
figure(2) ; clf
figure(4) ; clf
...
for iCondition = 1:nCondition
    ...
    for iTrial = 1:nTrial
        ...
        if ismember(Result.CueText, ExcludedCueTextArray), continue, end   % NoGo・Stop 除外
        if isempty(Result.NetVelTop),     continue, end
        if isnan(Result.TCueMarker),      continue, end
...
figure(2)
sgtitle(sprintf('Subject %02d：合成速度の時系列（NoGo・Stop 除外）', iSubject)) ;

figure(4)
sgtitle(sprintf('Subject %02d：投手方向成分 Vx の時系列（NoGo・Stop 除外）', iSubject)) ;
