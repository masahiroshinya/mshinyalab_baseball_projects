...
%% プロトコール
% パラメータ：　試行ごとに異なる
Protocol(1).Foreperiod = 2.0 ;
Protocol(1).CueText = "go" ;

Protocol(2).Foreperiod = 1.0 ;
Protocol(2).CueText = "nogo" ;
...
%
nTrial = length(Protocol) ;

for iTrial = 1:nTrial
    
    % 何かのキーを押すまで待機
    msg = [sprintf('%d試行目: ', iTrial), Protocol(iTrial).CueText, '  何かのキーを押してください'] ;
    disp(msg)
    pause
    
    %
    switch Protocol(iTrial).CueText
        case "go"
    ...
    waveform0 = [voltage_off * ones(ini_duration * sample_rate,  1);... % 最初のゼロ
        ...
        voltage_off * ones((Protocol(iTrial).Foreperiod - ready_signal_duration + gonogo_signal_duration + off_duration) * sample_rate,  1)] ; % 残り

    waveform1 = [voltage_off * ones((ini_duration + trig_signal_duration + trig_to_ready_interval + Protocol(iTrial).Foreperiod) * sample_rate,  1);... % 最初のゼロ
        voltage_ao1 * ones(gonogo_signal_duration * sample_rate,  1);... % Go/Nogo cue
