...
本研究は、4 種類の実験課題（自由スイング・単純反応課題・Go/Nogo 課題・Go/Stop 課題）を通じて、認知的な判断の難しさを段階的に変化させ、バットスイング動作にどのような変化が生じるかを定量的に調べることを目的としています。
...
実験は広島大学総合科学部棟 B107 スポーツバイオメカニクス実験室で行います。実験課題は、フォースプレート（床反力計）の上に立った状態で、LED による光刺激（白色・緑色・赤色）に応じてバットを全力でスイングする、またはスイングをしない・中断するというものです。課題は以下の 4 条件で構成されます。

- **自由スイング**：Ready cue（白色 LED）に続いて、自分のタイミングで全力スイングを行う条件
- **単純反応課題**：Go cue（緑色 LED）が点灯したら、できるだけ素早く全力スイングを行う条件
- **Go/Nogo 課題**：Go cue（緑色 LED）が点灯したらスイング、Nogo cue（赤色 LED）が点灯したらスイングをしない条件
- **Go/Stop 課題**：Go cue（緑色 LED）が点灯したらスイングを開始し、その後 Stop cue（赤色 LED）が続いて点灯した場合はスイングを途中で止める条件

各条件につき 20 試行を実施します。条件の実施順序は参加者間でカウンターバランスをとります。

【測定項目・使用機材】

身体各部位の動きを計測するために、反射マーカーを身体（肩・腰・膝・足首等）およびバットに貼付した上で、三次元モーションキャプチャーシステム（Qualisys）を使用します。また、フォースプレートにより床反力を計測します。LED 刺激呈示には NI DAQ ボードおよび自作 LED 基板を使用します。これらはいずれも非侵襲的な計測手法であり、痛みはありません。なお、実験の様子はデジタルビデオカメラで撮影します。
...
この研究は、広島大学総合科学部棟 B107（スポーツバイオメカニクス実験室）において、倫理審査委員会の承認を得た日から XXXX 年 XX 月 XX 日まで実施される予定です（研究期間は確定後に記入）。参加者の方に実験に参加していただくのは 1 日（最大 160 分）です。
