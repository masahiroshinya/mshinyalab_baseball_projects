...
% 単純選択反応課題テスト用スクリプト
% ao0：緑LED（刺激）+ Qualisys Syncトリガ（並列接続）

%% ===== 1. 設定セクション =====
disp('--- 単純選択反応課題テスト 初期設定 ---');

csv_file = '../3.1_プロトコールCSV設計/protocol_sample.csv';
device_id = "Dev1";
ch_g = "ao0";       % ao0：緑LED + Qualisysトリガ（並列）
...
test_tbl = protocol_tbl(41:47, :);   % simple_reaction 練習 7試行
fprintf('Trial 41〜47（単純選択反応 練習 7試行）を実行します。\n');
fprintf('ISIオフセット: +%.1f秒　初回待機: 5秒　推定総時間: 約67秒\n\n', isi_offset);

%% ===== 3. DAQセットアップ =====
disp('DAQのセットアップ中...');
try
    dq = daq("ni");
    addoutput(dq, device_id, ch_g, "Voltage");
    write(dq, 0.0);
    disp('DAQのセットアップ完了。');
catch ME
...
try
    for i = 1:run_trials
        ...
        isi_time = test_tbl.isi_sec(i) + isi_offset;   % ISI + 5秒オフセット
        led_dur  = test_tbl.led_duration_sec(i);
        v_on     = test_tbl.voltage_on(i);

        volt_out = 0.0;
        if v_on > 0
            if strcmp(color, "white") || strcmp(color, "green")
                volt_out = v_on;
            end
        end

        fprintf('Trial %d/%d: color=%s, ISI=%.2fs, Dur=%.3fs\n', ...
                i, run_trials, color, isi_time, led_dur);
