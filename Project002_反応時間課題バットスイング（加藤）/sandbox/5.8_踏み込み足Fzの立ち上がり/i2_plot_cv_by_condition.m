...
for im = 1:nM
    ...
    % 被験者ごとの CV（点と折れ線）
    for iS = 1:nS
        % ★ 色は i0 の SubjColor（i1・i4 と同じ被験者が同じ色になる）。
        %   この図は 1点 = 1被験者なので、色がないと点がどの被験者か分からない。
        plot(ax, 1:nC, C(iS,:), '-', 'Color', SubjColor(iS,:), 'LineWidth', 1.6, ...
            'Marker', 'o', 'MarkerSize', 7, 'MarkerFaceColor', SubjColor(iS,:), ...
            'MarkerEdgeColor', SubjColor(iS,:)*0.7) ;
    end
...
annotation('textbox', [0 (figH-78.75)/figH 1 33.6/figH], 'String', ...
    ['点 = 被験者1名の CV（その条件の全試行から算出） / 色つきの折れ線 = 同一被験者（色は被験者に対応） / ' ...
